...
# define OOMPH_LIBFABRIC_LOCALITY_SIZE 48
#endif

#if defined(OOMPH_LIBFABRIC_VERBS) || \
    defined(OOMPH_LIBFABRIC_TCP) || \
    defined(OOMPH_LIBFABRIC_SOCKETS) || \
    defined(OOMPH_LIBFABRIC_PSM2)
# define OOMPH_LIBFABRIC_LOCALITY_SIZE 16
# define OOMPH_LIBFABRIC_LOCALITY_SOCKADDR
#endif
...
namespace locality_defs {
    ...
    const uint32_t array_size   = OOMPH_LIBFABRIC_LOCALITY_SIZE;
    const uint32_t array_length = OOMPH_LIBFABRIC_LOCALITY_SIZE/4;
}
...
struct locality {
    ...
    explicit locality(const locality_data &in_data)
    {
        ...
        fi_address_ = 0;
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("expl constructing"), iplocality((*this))));
    }

    locality() {
        std::memset(&data_[0], 0x00, locality_defs::array_size);
        fi_address_ = 0;
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("default construct"), iplocality((*this))));
    }
    ...
    {
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("copy construct"), iplocality((*this))));
    }
    ...
    locality(const locality &other, fi_addr_t addr)
        ...
        , fi_address_(addr)
    {
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("copy fi construct"), iplocality((*this))));
    }
    ...
    locality(locality &&other)
        ...
        , fi_address_(other.fi_address_)
    {
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("move construct"), iplocality((*this))));
    }

    // provided to support sockets mode bootstrap
    explicit locality(const std::string &address,  const std::string &portnum)
    {
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("explicit construct")
            , address, ":", portnum));
        //
        ...
        fi_address_ = 0;
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("string constructing"), iplocality((*this))));
    }

    // some condition marking this locality as valid
    explicit inline operator bool() const {
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("bool operator"), iplocality((*this))));
        return (ip_address() != 0);
    }

    inline bool valid() const {
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("valid operator"), iplocality((*this))));
        return (ip_address() != 0);
    }
    ...
    locality & operator = (const locality &other) {
        data_       = other.data_;
        fi_address_ = other.fi_address_;
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("copy operator")
            , iplocality(*this)
            , iplocality(other)));
        return *this;
    }
    ...
    bool operator == (const locality &other) {
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("equality operator")
            , iplocality(*this)
            , iplocality(other)));
    ...
    bool less_than(const locality &other) {
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("less operator")
            , iplocality(*this)
            , iplocality(other)));
...
#if defined (OOMPH_LIBFABRIC_LOCALITY_SOCKADDR)
        return reinterpret_cast<const struct sockaddr_in*>
            (data_.data())->sin_addr.s_addr;
#elif defined(OOMPH_LIBFABRIC_GNI)
        return data_[0];
#else
...
private:
    friend bool operator==(locality const & lhs, locality const & rhs) {
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("equality friend")
            , iplocality(lhs)
            , iplocality(rhs)));
    ...
    friend bool operator<(locality const & lhs, locality const & rhs) {
        ...
        OOMPH_DP_ONLY(loc_deb, trace(hpx::debug::str<>("less friend")
            , iplocality(lhs)
            , iplocality(rhs)));
